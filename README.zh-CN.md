...
- 可在同一主体、不同候选的已批准断言之间人工确认冲突、补充、替代或作废关系；关系独立于不可变候选保存，记录两端快照与确认/撤销历史，撤销后可恢复此前画像。
...
Echo Memory 默认关闭。进入设置页“记忆提取”阶段后：

1. 在“记忆工作区”中设置 Vault 内的根目录，默认是 `Echo Memory`。
2. 执行初始化，只填写称呼、当前角色和近期目标。初始化完成后会自动启用记忆提取，目录语言按当时的界面文案语言固定。
...
每个候选包同时包含便于阅读的 Markdown 表格和插件托管的 JSON 数据。候选包保持模型提取时的内容，不因审核而改写；同目录审核 sidecar 使用 `echo-memory-review:managed` 托管区块记录逐条状态、生效内容、备注、审核时间和完整事件历史，区块外人工正文会保留。输入指纹由转写正文、纳入的成功纪要、Schema、提示词版本、输出语言、初始化用户、Provider 和模型共同生成；相同输入会复用已有候选，不会重复调用模型。旧候选首次审核或重建时会创建全为 `pending` 的 sidecar，不会静默批准。画像只替换 `echo-memory:managed` 标记之间的内容，标记外的人工正文会保留。
...
执行“Create personal agent context package”会先打开生成前预览。用户可以按项目、人物、起止日期筛选当前已批准且关系解析后的记忆，并设置 4,000～100,000 字符预算（默认 12,000）；项目和人物筛选使用 OR 语义，结果按最新观察时间优先排列，超出预算的条目会明确显示省略数量。确认后文件写入 `06 上下文包`，每条事实保留证据、transcript、候选、审核和关系回链，重复生成只更新 `echo-memory-context:managed` 区块并保留区块外人工正文。该流程默认完全本地，不会发送笔记或调用外部 Agent。
...
当前 MVP 不包含外部 Agent CLI、语义检索、向量数据库、跨 Vault 同步或自动执行日历/笔记动作；上下文包只提供本地、可预览的 Markdown。

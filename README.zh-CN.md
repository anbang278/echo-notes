...
- 支持“会议页 + 候选包”和“会议页 + 候选包 + 自动编译画像”两种沉淀模式；候选包是真源，User、人物、组织和项目画像只更新托管区块。
- 可从候选包完整重建画像；重建只遍历 Echo Memory 根目录，不扫描整个 Vault，也不覆盖人工正文。
...
4. 在“编译策略”中选择沉淀模式。默认只生成会议页与候选包；自动编译模式会继续更新 User、人物、组织和项目画像。
5. 长文本默认按 24,000 字符的自然边界顺序提取，最多 20 块；画像默认只编译置信度不低于 `0.75` 且带可定位原文证据的断言。
...
└── 99 系统/echo-memory.json、运行日志/
```

每个候选包同时包含便于阅读的 Markdown 表格和插件托管的 JSON 数据。输入指纹由转写正文、纳入的成功纪要、Schema、提示词版本、输出语言、初始化用户、Provider 和模型共同生成；相同输入会复用已有候选，不会重复调用模型。画像只替换 `echo-memory:managed` 标记之间的内容，标记外的人工正文会保留。
...
当前 MVP 不包含外部 Agent CLI、向量数据库、跨 Vault 同步或自动执行日历/笔记动作。
...
单个 AI 分析任务最长运行 15 分钟；长文本的分块提取与最终汇总共享这一时限。超时后任务会自动标记为失败，在转写稿中保留错误状态，并可从任务中心重试。
...
Echo Notes 的长期目标，是从录音转写工具逐步演进为个人 AI Memory Layer。Echo Memory MVP 已验证单次会议的候选提取与画像编译闭环，后续会重点探索：
...
- Echo Memory MVP 只支持通用记忆 Schema；暂不提供 PM 等角色包、候选审核界面、向量检索和外部 Agent 动作。

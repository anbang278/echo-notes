...
- 离线模式仅保留已验证的阿里百炼、硅基流动、Ollama 和 LM Studio，用于转写 Vault 中已有音频。
- 转写当前笔记中选中的音频链接。
- 扫描并转写当前笔记中的全部支持音频链接。
...
默认转写语言只会发送给支持语言参数的 Provider，例如阿里百炼、Ollama 和 LM Studio。AgentPlan 说话人分离只使用中文或省略 language；选择其他语言时会自动切换为 `auto`。SiliconFlow 官方转写接口只声明 `file` 和 `model`，因此 Echo Notes 不会向它发送非标准语言字段，仍由模型自动识别。
...
4. 离线模式：选择阿里百炼、硅基流动、Ollama 或 LM Studio，确认 Base URL、Model、API Key 和默认语言。

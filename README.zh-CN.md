...
进入 **Echo Notes → 录音转写 → 输出规则 → 录音存放位置**。该配置同时适用于 Echo Notes 实时录音，以及核心录音机的快捷键、命令面板和麦克风按钮。

| 策略 | 位置 |
| --- | --- |
| 跟随 Obsidian 附件设置（默认） | 使用 Obsidian 原有附件目录规则 |
| 当前笔记同目录 | 所属笔记所在的目录 |
| 当前笔记下指定子目录 | 所属笔记目录下的指定子目录，例如 `Recordings` 或 `附件/录音` |
| 库内固定目录 | Vault 根目录下的指定目录，例如 `Recordings`；`.` 表示根目录 |

目录支持中文和多级路径，不存在时自动创建；文件重名时自动编号。请使用库内相对路径，不接受绝对路径、URL、`..`、空目录或非法路径。目录输入移出焦点或按 Enter 后保存，非法输入会显示错误并保留原配置。

核心录音按**保存时**正在查看的笔记确定目录并插入链接；没有来源笔记时，同目录和子目录策略以库根目录为基准，并在保存后打开音频。实时录音按**开始时**的笔记和配置确定目录，录音期间切换笔记或修改配置不会改变当前录音的位置。

设置仅影响新录音，不会移动历史文件。下方“输出目录策略”独立控制转写稿位置，录音的新位置会作为其相对路径基准。Echo Notes 停用后核心录音机恢复原生保存；核心保存接口不兼容或指定目录保存失败时，会提示并尝试按 Obsidian 附件规则保存。音频已保存但链接插入失败时，不会重复创建录音。

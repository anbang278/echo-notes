...
5. Keep long-text chunking enabled for large inputs. The default chunk size is 24,000 characters with a maximum of 20 chunks. New and legacy candidates start pending; only explicitly approved assertions enter profiles, and their effective values can be corrected during review. New candidates use the v2 memory taxonomy (six memory types plus a `working` or `long_term` horizon, an admission reason, and optional validity dates); working memories participate in the timeline, cross-record views, and context packages but do not enter stable profiles.
...
Each candidate package contains a readable Markdown table and plugin-managed JSON and remains unchanged by review. New candidates use Schema v2 and classify each assertion by one of six memory types (`fact`, `decision`, `preference`, `belief`, `experience`, `goal`) and a `working` or `long_term` horizon, optionally with a short admission reason and evidence-backed validity dates. Legacy Schema v1 candidates are still read and reviewed without migration; their assertions keep an unclassified type and are treated as long-term for profile compatibility. Its same-directory review sidecar stores per-assertion status, effective value, note, review time, and full event history inside an `echo-memory-review:managed` block while preserving manual text outside that block. Its input fingerprint covers the transcript body, included analyses, schema, prompt, and pipeline versions, output language, initialized user, Provider, and model. Repeating the same input reuses the existing candidate without calling the model again. A legacy candidate receives a fully pending sidecar when first reviewed or rebuilt and is never silently approved. Profile compilation consumes only approved long-term assertions (or legacy assertions without a horizon), replaces content between `echo-memory:managed` markers, and preserves user-authored text outside those markers; working memory stays in the timeline, cross-record views, and context packages.
...
Run **Create personal agent context package** to preview and generate a local Markdown context package under `06 上下文包` (or `06 Context Packages`). Project and person filters use OR semantics; optional start/end dates filter the assertion `observedAt` day, and optional memory-type and memory-horizon filters narrow the included assertions (an unset filter means any value, and legacy assertions without a horizon are treated as long-term). The character budget is bounded to 4,000-100,000 characters with a 12,000-character default. Entries are ordered newest-first, preserve evidence plus transcript/candidate/review/relation backlinks, and show how many matching entries were omitted by the budget. Generation uses only approved, relation-resolved memory, updates only the `echo-memory-context:managed` block, preserves manual text outside the block, and makes no network request or external Agent call.
...
The current MVP does not include external Agent CLIs, semantic retrieval, a vector database, cross-vault sync, automatic calendar and note actions, automatic memory approval, automatic relation discovery, or the full working-memory expiry lifecycle; context packages are local, previewable Markdown only.
